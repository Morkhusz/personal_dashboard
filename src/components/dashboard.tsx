...
function StickyNotes() {
  ...
  return <><div className="fixed inset-0 z-40 pointer-events-none">{notes.map((note,index)=><article key={note.id} className={cn("pointer-events-auto fixed h-[180px] w-[180px] touch-none select-none border-2 bg-sticky text-sticky-foreground shadow-xl transition-[border-color,box-shadow]",note.ringing?"animate-pulse border-red shadow-glow-red":"border-sticky/80")} style={{left:note.x,top:note.y,zIndex:41+index,transform:"rotate(-1deg)"}} onPointerDown={beginMove(note)} onPointerMove={move} onPointerUp={stopDrag} onDoubleClick={()=>setEditingId(note.id)}>
    ...
    <div className="absolute inset-x-4 bottom-10 top-8 grid place-items-center overflow-hidden text-center font-semibold leading-tight" style={{fontSize:`${fontSize(note.text)}px`}}>{editingId===note.id?<Textarea autoFocus aria-label="Texto do post-it" className="h-full min-h-0 resize-none border-0 bg-transparent p-0 text-center text-inherit shadow-none focus-visible:ring-0" value={note.text} maxLength={500} onPointerDown={event=>event.stopPropagation()} onChange={event=>setNotes(current=>current.map(item=>item.id===note.id?{...item,text:event.target.value}:item))} onBlur={()=>setEditingId(null)}/>:note.text||<span className="text-sm font-medium opacity-55">Clique duas vezes para escrever</span>}</div>
    <Button variant="ghost" size="iconSm" className={cn("absolute bottom-1 left-1/2 -translate-x-1/2 text-sticky-foreground hover:bg-sticky-foreground/10",note.alarmAt&&!note.ringing&&"bg-sticky-foreground/10")} onClick={()=>note.ringing?stopAlarm(note.id):(setAlarmId(note.id),setMinutes("5"))} aria-label={note.ringing?"Parar notificação":"Definir notificação"}>{note.ringing?<Square className="fill-current"/>:<Clock3/>}</Button>
  </article>)}</div>
