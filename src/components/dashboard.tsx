...
export function Dashboard() {
  ...
  const [tabs,setTabs]=useState<DashboardTab[]>([{id:"main",name:"Principal",positions:defaults,hiddenWidgets:[]}]);
  ...
  useEffect(()=>{
    ...
    if(savedTabs){
      try {
        ...
        if(parsed.tabs?.length){setTabs(parsed.tabs.map(tab=>({...tab,positions:{...defaults,...tab.positions},hiddenWidgets:tab.hiddenWidgets??[]})));setActiveTabId(parsed.tabs.some(tab=>tab.id===parsed.activeTabId)?parsed.activeTabId??parsed.tabs[0].id:parsed.tabs[0].id)}
      } catch { localStorage.removeItem(TABS_STORAGE_KEY); }
    } else {
      const legacy=localStorage.getItem(LEGACY_STORAGE_KEY);
      if(legacy){try{setTabs([{id:"main",name:"Principal",positions:{...defaults,...JSON.parse(legacy)},hiddenWidgets:[]}])}catch{localStorage.removeItem(LEGACY_STORAGE_KEY)}}
    }
    setHydrated(true);
  },[defaults]);
  ...
  const addTab=()=>{const id=`dashboard-${Date.now()}`;const tab={id,name:`Dashboard ${tabs.length+1}`,positions:{...defaults},hiddenWidgets:[]};setTabs(old=>[...old,tab]);setActiveTabId(id);setEditable(false)};
